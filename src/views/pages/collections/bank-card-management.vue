<template>
  <Layout>
    ...
    <BRow>
      <Breadcrumb :title="$t('t-bank-card-management')" />

 
        <BCard no-body class="ctx-content px-0 card-body">
          <BCardBody>
            <div>
              <form class="d-flex flex-row flex-wrap align-items-center gap-2">
                ...
                <div class="col-12 col-lg-auto">
                  <input type="text" class="form-control text-form" id="placeholderInput" :placeholder="$t('t-account-holder-name')" />
                </div>

                <div class="d-flex gap-2">
                  <button type="button" class="btn btn-secondary waves-effect waves-light mx-2 p-2 text-form">
                    {{ $t('t-search') }}
                  </button>
                  <button type="button" class="btn btn-light waves-effect p-2 text-form">
                    {{ $t('t-reset') }}
                  </button>
                  <button type="button" class="btn btn-warning waves-effect p-2 text-form" @click="onOpenAddBankCard">
                    {{ $t('t-add-bank-card') }}
                  </button>
                </div>
                  ...
                  <tr v-for="(item, index) in items" :key="index">
                    ...
                    <td>
                      <button
                        ...
                        type="button"
                        :class="item.status === 'active'?'btn-customer-green':'btn-customer-ok'"
                        v-on:click.prevent=""
                      >
                    ...
                    <td>
                      <button
                        style="margin-left: 2px"
                        type="button"
                        :class="item.script_execution_status === 'active'?'btn-customer-green':'btn-customer-ok'"
                        v-on:click.prevent=""
                      >
                        {{ item.script_execution_status === 'active'?"启用":"停用脚本"}}
                      </button>
                    </td>
                    ...
                    <td class="group-button">
                        ...
                        v-on:click.prevent="onOpenModalBankCardEdit(index)"
                      >
                      {{ $t('t-edit') }}
                      </button>
                        ...
                        v-on:click.prevent="handleChangeStatus(index)"
                      >
                        {{ item.status === 'active'? $t('t-deactivate') : $t('t-enable') }}
                      </button>
                        <button
                        style="margin-left: 2px"
                        type="button"
                        :class="item.script_execution_status === 'active'?'btn-customer-ok':'btn-customer-green'"
                        v-on:click.prevent="handleChangeScriptExecution(index)"
                      >
                        {{ item.script_execution_status === 'active'? $t('t-deactivate-script') : $t('t-enable-script') }}
                      </button>
