<template>
    <Layout>
          ...
          <BCardBody>
            <div class="mt-2 p-3">
              <form class="d-flex flex-row flex-wrap align-items-center gap-2">
                ...
                <div class="col-12 col-lg-auto">
                  <input type="text" class="form-control" id="placeholderInput" :placeholder="$t('名称')" />
                </div>

                <div class="col-12 col-lg-auto">
                  <input type="text" class="form-control" id="placeholderInput" :placeholder="$t('搜索')" />
                </div>
                ...
                <div class="d-flex gap-2">
                  ...
                  @click="onOpenModalVpnAdd"
                  >
                    {{ $t('添加VPN') }}
                  </button>
                  <button type="button" class="btn btn-secondary waves-effect p-2"
                  @click="onOpenModalHint"
                  >
                    {{ $t('VPN批量检测') }}
                  </button>
                </div>
                  ...
                  <tr v-for="(item, index) in items" :key="index">
                    ...
                    <td class="d-flex gap-3">
                      <button type="button" class="btn btn-info waves-effect waves-light btn-row active px-2"
                        @click="onOpenModalVpnEdit(index)">
                        {{ $t('t-edit') }}
                      </button>
        ...
        <ModalVpn v-model="openModalVpn" :on-confirm="handleUpdate" :mode="modeModal"  />
        <ModalHint v-model="openModalHint" :on-confirm="handleCloseChanel" :on-close="onClose"/>
    </Layout>
</template>
...
import { ref } from "vue";
import ModalVpn from "../../../components/modals/ModalVpn.vue";
...
const openModalVpn = ref(false);
const modeModal = ref("");
const openModalHint = ref(false);

const onOpenModalVpnEdit = (index) => {
  itemSelected.value = index;
  openModalVpn.value = true;
  modeModal.value = "edit"
};

const onOpenModalVpnAdd = () => {
  openModalVpn.value = true;
  modeModal.value = "add"
}

const onOpenModalHint = () => {
  openModalHint.value = true;
};

const onClose = () => {
  openModalHint.value = false;
};
